...
class SimpleRapidFireTrainer:
    ...
    def _update_signal_weights(
        self,
        successful_signals: Dict[str, List[float]],
        unsuccessful_signals: Dict[str, List[float]],
    ) -> None:
        """
    ...
    def visualize_performance(self) -> None:
        ...
        try:
            ...
            plt.subplot(1, 2, 1)
            plt.plot(
                [t.timestamp() for t in self.dashboard_data["timestamp"]],
                self.dashboard_data["capital"],
                "b-",
            )
