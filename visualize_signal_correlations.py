...
def generate_correlation_matrix():
    ...
    ax.add_patch(plt.Rectangle((0, n - n_technical), n_technical, n_technical, fill=True, color=tech_color, alpha=0.3, zorder=-1))
    ax.add_patch(plt.Rectangle((0, n - n_technical - n_logistics), n_technical, n_logistics, fill=True, color=tech_color, alpha=0.15, zorder=-1))
    ax.add_patch(plt.Rectangle((0, 0), n_technical, n_macro, fill=True, color=tech_color, alpha=0.05, zorder=-1))
    
    ax.add_patch(plt.Rectangle((n_technical, n - n_technical - n_logistics), n_logistics, n_logistics, fill=True, color=logistics_color, alpha=0.3, zorder=-1))
    ax.add_patch(plt.Rectangle((n_technical, 0), n_logistics, n_macro, fill=True, color=logistics_color, alpha=0.15, zorder=-1))
    
    ax.add_patch(plt.Rectangle((n_technical + n_logistics, 0), n_macro, n_macro, fill=True, color=macro_color, alpha=0.3, zorder=-1))
    
    # Add category annotations
    plt.figtext(0.15, 0.03, "Technical Signals", fontsize=12, ha="center", bbox={"facecolor":tech_color, "alpha":0.5, "pad":5})
    plt.figtext(0.5, 0.03, "Logistics Signals", fontsize=12, ha="center", bbox={"facecolor":logistics_color, "alpha":0.5, "pad":5})
    plt.figtext(0.85, 0.03, "Macro Signals", fontsize=12, ha="center", bbox={"facecolor":macro_color, "alpha":0.5, "pad":5})
    ...
    plt.figtext(0.5, 0.95, legend_text, fontsize=10, ha="center", bbox={"facecolor":"white", "alpha":0.8, "pad":5})
    
    # Calculate average correlation
    avg_corr = (correlation_matrix.sum() - n) / (n * n - n)
    plt.figtext(0.5, 0.01, f"Average Correlation: {avg_corr:.3f} (Hedge Funds target < 0.3)", fontsize=12, ha="center", bbox={"facecolor":"white", "alpha":0.8, "pad":5})
