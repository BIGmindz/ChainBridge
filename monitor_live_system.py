        ...
        try:
            if os.path.exists("budget_state.json"):
                with open("budget_state.json", "r") as f:
                    ...
                    self.trading_metrics["available_capital"] = budget_data.get("available_capital", self.trading_metrics["available_capital"])
                    self.trading_metrics["allocated_capital"] = budget_data.get("allocated_capital", self.trading_metrics["allocated_capital"])
    ...
    def print_dashboard(self):
        ...
        if current_listings:
            ...
            for i, listing in enumerate(current_listings[:5], 1):
                print(f"{i}. {listing.get('coin', 'N/A')} on {listing.get('exchange', 'N/A')} - " +
