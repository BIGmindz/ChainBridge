...
class RapidFireMLTrainer:
    ...
    def collect_signal_data(self) -> np.ndarray:
        ...
        for symbol, data in self.bot.last_signals.items():
            ...
            for signal_name in ['Port_Congestion', 'Diesel', 'Supply_Chain', 'Container']:
                if signal_name in module_signals:
                    signal = module_signals[signal_name]
                    if signal == 'BUY':
                        signal_values.append(1.0)
            ...
            for signal_name in ['Inflation', 'Regulatory', 'Remittance', 'CBDC', 'FATF']:
                if signal_name in module_signals:
                    signal = module_signals[signal_name]
                    if signal == 'BUY':
                        signal_values.append(1.0)
    ...
    def record_decision(self, signals: np.ndarray, action: str, 
