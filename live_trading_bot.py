...
class LiveTradingBot:
    def __init__(self):
        ...
        # Trading symbols
        self.symbols = ["WIF/USD", "DOGE/USD", "NEAR/USD", "SOL/USD", "PEPE/USD"]
        ...
        for symbol in self.symbols:
            try:
                ...
                if self.budget_manager.available_capital < min_trade_size:
                    print(f"⚠️  Insufficient capital for {symbol} (need ${min_trade_size}, have ${self.budget_manager.available_capital:.2f})")
                    continue
    ...
    def _generate_multi_signal(self, symbol, ohlcv, price):
        ...
        try:
            ...
            # Logistics signal
            module_signals["LogisticsSignal"] = self._process_module(self.logistics, "LogisticsSignal", {"price_data": price_data, "symbol": symbol})
            
            # Global Macro signal
            module_signals["GlobalMacro"] = self._process_module(self.global_macro, "GlobalMacro", {"price_data": price_data, "symbol": symbol})
            ...
            module_signals["RegionCryptoMapper"] = self._process_module(self.region_crypto, "RegionCryptoMapper", {"symbol": symbol, "macro_signals": macro_test_signals})
