...
def plot_cv_folds(splits, X):
    ...
    for i, (train_idx, test_idx) in enumerate(splits):
        ...
        train_times = X.index[train_idx]
        plt.fill_between(train_times, i-0.4, i+0.4,
                        color=colors[i % len(colors)], alpha=0.3,
                        label=f'Fold {i+1} Train' if i == 0 else "")

        # Plot test periods
        test_times = X.index[test_idx]
        plt.fill_between(test_times, i-0.4, i+0.4,
...
if __name__ == "__main__":
    ...
    plt.savefig('/Users/johnbozza/bensonbot/Multiple-signal-decision-bot/purged_kfold_demo.png',
