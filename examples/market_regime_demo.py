...
class MarketRegimeSimulation:
    ...
    async def run_simulation(self):
        ...
        for hour in range(self.hourly_intervals):
            ...
            # Show progress
            if hour % 24 == 0:
                print(f"Day {hour//24 + 1}: Price ${current_price:,.2f}, " 
        ...
        for regime, perf in stats.get('regime_performance', {}).items():
            if perf['trades'] > 0:
                print(f"{regime.upper()}: {perf['trades']} trades, "
    ...
    def visualize_results(self):
        ...
        for hour, regime in enumerate(self.detected_regimes):
            if regime != current_regime or hour == len(self.detected_regimes) - 1:
                if current_regime is not None:
                    ax1.axvspan(start_hour, hour, alpha=0.2, color=regime_colors.get(current_regime, 'lightgray'))
        ...
        if has_matplotlib:
            try:
                ...
                legend_elements = [Patch(facecolor=color, alpha=0.2, label=regime)
    ...
    def _visualize_signal_regime_performance(self):
        ...
        for i, regime in enumerate(regimes):
            ...
            ax1.bar(x + i*width - width*(len(regimes)-1)/2, win_rates, width, label=regime)
        ...
        for i, regime in enumerate(regimes):
            ...
            ax2.bar(x + i*width - width*(len(regimes)-1)/2, pnl_values, width, label=regime)
