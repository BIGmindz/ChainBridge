...
1. **Do NOT use the old keys**; update environment variables in your deployment pipeline and any local `.env` files.

1. **Remove `.env.backup` from git history** (see Purging history below).

1. **Do not commit .env or other secrets** — keep them in a secret manager.
