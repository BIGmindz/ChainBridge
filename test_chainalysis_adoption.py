...
def test_adoption_tracker():
    ...
    try:
        ...
        plt.title('Chainalysis Global Crypto Adoption Index - Regional Growth Rates', fontsize=14)
        plt.grid(axis='x', linestyle='--', alpha=0.7)
        
        # Add legend
        green_patch = plt.Rectangle((0, 0), 1, 1, fc='#5cb85c')
        gray_patch = plt.Rectangle((0, 0), 1, 1, fc='#6c757d')
        plt.legend([green_patch, gray_patch, plt.Line2D([0], [0], color='red', linestyle='--')], 
        ...
        plt.annotate(f"{result['signal']} Signal ({result['confidence']*100:.0f}% Confidence)",
                    xy=(0.5, 0.97), xycoords='figure fraction',
                    bbox=signal_box, ha='center', fontsize=12)
