...
def load_and_prepare_data(data_dir="data/"):
    ...
    df_list = [pd.read_csv(file) for file in all_files if os.path.getsize(file) > 0]
    if not df_list:
        return None
...
def create_candlestick_chart(df, symbol):
    ...
    # Moving Averages
    fig.add_trace(go.Scatter(x=resampled.index, y=resampled['ma_10'], mode='lines', 
                             line=dict(color=COLORS['primary'], width=1), name='10-period MA'), row=1, col=1)
    fig.add_trace(go.Scatter(x=resampled.index, y=resampled['ma_50'], mode='lines',
                             line=dict(color=COLORS['gold'], width=1), name='50-period MA'), row=1, col=1)
    ...
    fig.add_trace(go.Bar(x=resampled.index, y=vol_series, marker_color=volume_colors, name='Volume'), row=2, col=1)
...
def create_dashboard():
    ...
    col1.metric("Portfolio Value", f"${latest_row.get('portfolio_value', 0):,.2f}", f"{pnl:,.2f} ({pnl_pct:.2f}%)")
    ...
    with col2:
        ...
        signal_cols = ['price', 'rsi_value', 'ob_imbalance', 'vol_imbalance', 'ml_signal', 'combined_score']
        ...
        if present_cols:
            ...
            fig.update_layout(template="plotly_dark", paper_bgcolor=COLORS['background'], plot_bgcolor=COLORS['background'])
