    ...
    try:
        while time.time() < end_time:
            ...
            performance_snapshots.append(
                {"timestamp": timestamp, "metrics": performance}
            )
            ...
            secs = remaining % 60
            print(
                f"Waiting {wait_time:.1f}s before next collection. "
...
def prepare_data_for_later_training(session_dir):
    ...
    summary = {
        ...
        "symbols_covered": list(
            set(
                snapshot.get("data", {}).get("symbol", "unknown")
                for snapshot in signal_data
                if "data" in snapshot
            )
        ),
