...
def parse_args():
    ...
    parser.add_argument("--adaptive-config", type=str, default="config/adaptive_weight_config.yaml",
    ...
    try:
        while time.time() < end_time:
            ...
            if len(signal_data_snapshots) % 10 == 0:
                save_collected_data(session_dir, signal_data_snapshots, 
                                   market_condition_snapshots, performance_snapshots)
            ...
            mins = remaining // 60
            secs = remaining % 60
            print(f"Waiting {wait_time:.1f}s before next collection. "
                  f"Remaining time: {mins}m {secs}s")
...
def prepare_data_for_later_training(session_dir):
    ...
    summary = {
        ...
        "symbols_covered": list(set(snapshot.get("data", {}).get("symbol", "unknown") 
                            for snapshot in signal_data if "data" in snapshot)),
        "market_regimes_detected": list(set(snapshot.get("condition", "unknown") 
                                  for snapshot in market_condition if "condition" in snapshot)),
    }
...
def generate_basic_visualizations(session_dir):
    ...
    with open(os.path.join(viz_dir, "data_collection_results.json"), "w") as f:
        json.dump({
            ...
            "market_regimes_detected": list(set(snapshot.get("condition", "unknown") 
                                    for snapshot in market_condition if "condition" in snapshot)),
            "market_regime_counts": {regime: regimes.count(regime) for regime in set(regimes)} 
                                    if 'regimes' in locals() else {},
