...
class MarketSimulator:
    ...
    def update_market(self, bias=0):
        ...
        if len(self.price_history) > 1:
            self.cycle_returns.append(
                (self.price_history[-1] / self.price_history[-2]) - 1
            )
...
async def run_minute_paper():
    ...
    for condition in MARKET_CONDITIONS:
        ...
        for cycle in range(condition["cycles"]):
            ...
            print(
                f"Price: ${market_data['price']:.2f} ({market_data['change_pct']:+.2f}%) | {market_data['timestamp'].strftime('%Y-%m-%d %H:%M')}"
            )
