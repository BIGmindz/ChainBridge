            ...
            for symbol in self.symbols:
                try:
                    ...
                    module_signals["SentimentAnalysis"] = self._process_module(
                        self.sentiment, "SentimentAnalysis", {"symbol": symbol, "timeframe": self.timeframe}
                    )
                        ...
                        if position_calc['size'] > 0:
                            if agg_result["signal"] == "BUY":
                                ...
                                if budget_result['success']:
                                    ...
                                    self.exchange_adapter.place_order(symbol, "buy", position_calc['size'] / last_price, last_price)
                                else:
                                ...
                                if budget_result['success']:
                                    ...
                                    # Also execute the order in the exchange adapter for compatibility
                                    self.exchange_adapter.place_order(symbol, "sell", position_calc['size'] / last_price, last_price)
                                else:
