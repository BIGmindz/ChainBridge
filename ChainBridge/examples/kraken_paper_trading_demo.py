...
async def demo_basic_paper_trading():
    ...
    config = {
        ...
        "risk_management": {"max_position_size": 0.1, "max_drawdown_limit": 0.15},
        "logging": {"level": "INFO"},
    }
...
class MockMLIntegration:
    ...
    async def process_trading_signals(self, symbols):
        ...
        for symbol in symbols:
            ...
            if confidence >= self.config["confidence_threshold"] and action in [
                "BUY",
                "SELL",
            ]:
                ...
                try:
                    ...
                    results[symbol] = {"decision": decision, "execution": execution}
                except Exception as e:
                    results[symbol] = {"decision": decision, "error": str(e)}
            else:
                results[symbol] = {
                    "decision": {
                        "action": "HOLD",
    ...
    def get_signal_performance_report(self):
        ...
        return {"summary": {"active_modules": 4, "avg_success_rate": 0.67}}
