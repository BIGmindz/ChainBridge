...
def upgrade():
    ...
    if "ix_settlement_events_intent_occurred" not in existing_indexes:
        op.create_index(
            "ix_settlement_events_intent_occurred",
            "settlement_events",
            ["payment_intent_id", "occurred_at"],
            unique=False,
        )
    if "ix_settlement_events_intent_sequence" not in existing_indexes:
        op.create_index(
            "ix_settlement_events_intent_sequence",
            "settlement_events",
            ["payment_intent_id", "sequence"],
            unique=False,
        )
    if "ix_settlement_events_type_occurred" not in existing_indexes:
        op.create_index(
            "ix_settlement_events_type_occurred",
            "settlement_events",
    ...
    if "ix_payment_intents_shipment_latest_risk" not in pi_indexes:
        op.create_index(
            "ix_payment_intents_shipment_latest_risk",
            "payment_intents",
            ["shipment_id", "latest_risk_snapshot_id"],
            unique=False,
        )
