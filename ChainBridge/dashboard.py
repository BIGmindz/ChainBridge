...
def main():
    ...
    # Charts Row 2
    st.plotly_chart(create_signal_strength_chart(recent_df), use_container_width=True)
    ...
    if not display_df.empty:
        ...
        if display_cols:
            st.dataframe(display_df[display_cols].sort_values(by="timestamp", ascending=False), use_container_width=True)  # type: ignore
        else:
