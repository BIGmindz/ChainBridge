...
1. **Historical Data**: `data/consolidated_market_data.csv` must exist

1. **Strategy Configs**: Each strategy needs a valid `config.yaml`
...
1. **Equity Curve Chart** (`strategies/{strategy}/backtest_equity_curve.html`)
...
1. **Model Loading**: Loads trained ML models and scalers

1. **Feature Engineering**: Prepares features as defined in config

1. **Signal Generation**: Uses ML models to generate trading signals

1. **Trade Simulation**: Realistic execution with fees and position sizing

1. **Performance Analysis**: Calculates comprehensive metrics

1. **Report Generation**: Creates professional reports and visualizations
