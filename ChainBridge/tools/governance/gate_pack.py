...
# Governance Rule Registry (loaded on demand)
_governance_rules = None

def get_governance_rules() -> dict:
    """Load and cache the governance rule registry."""
    global _governance_rules
    if _governance_rules is None:
        try:
            if GOVERNANCE_RULES_PATH.exists():
                with open(GOVERNANCE_RULES_PATH, 'r') as f:
                    ...
                    for rule in _governance_rules["rules"]:
                        _governance_rules["by_error_code"][rule["error_code"]] = rule
                        _governance_rules["by_rule_id"][rule["rule_id"]] = rule
            else:
                _governance_rules = {"rules": [], "by_error_code": {}, "by_rule_id": {}}
        except Exception:
            _governance_rules = {"rules": [], "by_error_code": {}, "by_rule_id": {}}
...
def get_rule_by_error_code(error_code: str) -> dict:
    """Get a governance rule by its error code."""
    rules = get_governance_rules()
    return rules["by_error_code"].get(error_code)


def get_training_signal_for_error(error_code: str) -> dict:
    """Get training signal guidance for an error code."""
    rule = get_rule_by_error_code(error_code)
    if rule:
        return rule.get("training_signal", {})
    return {}
...
class ErrorCode(Enum):
    ...
    # Orchestrator Discipline (PAC-BENSON-P45-ORCHESTRATOR-IDENTITY-GATEWAY-AND-RESPONSE-DISCIPLINE-RESET-01)
    ...
    # Benson Execution Authority Enforcement (PAC-BENSON-P48-BENSON-EXECUTION-RUNTIME-AUTHORITY-AND-WRAP-GENERATION-01)
    GS_120 = "WRAP_AUTHORITY_VIOLATION — Only Benson (GID-00) may emit WRAP_ACCEPTED."
    GS_121 = "AGENT_WRAP_EMISSION_BLOCKED — Agents cannot emit WRAPs directly. Use EXECUTION_RESULT."
    GS_122 = "POSITIVE_CLOSURE_AUTHORITY_BLOCKED — Only Benson may declare POSITIVE_CLOSURE."
    GS_123 = "EXECUTION_RUNTIME_BYPASS — PAC execution must go through BENSON_EXECUTION runtime."
    GS_124 = "AGENT_SELF_CLOSURE_BLOCKED — Agent attempted self-closure without Benson authority."
    GS_125 = "WRAP_GENERATION_AUTHORITY_VIOLATION — WRAP can only be generated by Benson Execution."
    # Agent Execution Result Contract (PAC-DAN-P50-GOVERNANCE-EXECUTION-HANDOFF-AND-AGENT-RESULT-CONTRACT-01)
    ...
    # Governance Doctrine Enforcement (PAC-BENSON-P51-GOVERNANCE-DOCTRINE-V1-EXECUTION-INTELLIGENCE-FOUNDATION-01)
    GS_140 = "DOCTRINE_NOT_FOUND — GOVERNANCE_DOCTRINE_V1.md not present. Execution blocked."
    GS_141 = "RULE_REGISTRY_NOT_FOUND — governance_rules.json not present. Enforcement degraded."
    # Machine Learning Governance (PAC-BENSON-P51)
    GS_150 = "LEARNING_EVENT_NOT_RECORDED — Governance event not captured in ledger."
...
def validate_doctrine_presence() -> list:
    """
    HARD-GATE: GOVERNANCE DOCTRINE PRESENCE
    PAC-BENSON-P51-GOVERNANCE-DOCTRINE-V1-EXECUTION-INTELLIGENCE-FOUNDATION-01

    Validates that governance doctrine infrastructure is present.
    This is a foundational check that ensures the governance system
    is properly bootstrapped before any validation runs.

    Returns list of ValidationError if doctrine files missing.
    Mode: WARN (logs but doesn't block, as rule registry enhances but isn't required)
    """
    errors = []

    # Check doctrine file presence
    if not DOCTRINE_PATH.exists():
        errors.append(ValidationError(
            ErrorCode.GS_140,
            f"GOVERNANCE_DOCTRINE_V1.md not found at {DOCTRINE_PATH}. "
            "Governance execution proceeds with degraded authority model.",
            location=str(DOCTRINE_PATH)
        ))

    # Check rule registry presence (warn-only, doesn't block)
    if not GOVERNANCE_RULES_PATH.exists():
        errors.append(ValidationError(
            ErrorCode.GS_141,
            f"governance_rules.json not found at {GOVERNANCE_RULES_PATH}. "
            "Training signal guidance and machine learning integration degraded.",
            location=str(GOVERNANCE_RULES_PATH)
        ))
...
def validate_content(content: str, registry: dict) -> ValidationResult:
    ...
    # =========================================================================
    # SOFT-GATE: GOVERNANCE DOCTRINE PRESENCE (GS_140-GS_141)
    # PAC-BENSON-P51-GOVERNANCE-DOCTRINE-V1-EXECUTION-INTELLIGENCE-FOUNDATION-01
    #
    # Validates governance infrastructure is bootstrapped.
    # Mode: WARN (logs to console, does not block validation)
    # This is foundational — degraded doctrine = degraded authority model
    # =========================================================================
    doctrine_errors = validate_doctrine_presence()
    if doctrine_errors:
        # Log warnings but don't block (soft-gate)
        for err in doctrine_errors:
            print(f"⚠ DOCTRINE_WARN: {err.code.name} — {err.message}")
            # Store in warnings, not errors (doesn't fail validation)
    # Note: Can be promoted to hard-gate in future by extending errors instead
    # =========================================================================
    # END SOFT-GATE: GOVERNANCE DOCTRINE PRESENCE
    # =========================================================================
