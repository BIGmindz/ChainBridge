...
class SettlementLog(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    payment_intent_id = Column(Integer, nullable=False, index=True)
...
class PaymentSchedule(Base):
    ...
    description = Column(
        String(255),
        nullable=True,
        comment="Human-readable description of the schedule"
    )
...
class MilestoneSettlement(Base):
    ...
    amount = Column(
        Float,
        nullable=False,
        comment="Amount to settle for this milestone"
    )
    currency = Column(
        String(10),
        default="USD",
        nullable=False,
        comment="Currency for the settlement"
    )
    ...
    occurred_at = Column(
        DateTime,
        nullable=True,
        comment="When the triggering shipment event occurred"
    )
    settled_at = Column(
        DateTime,
        nullable=True,
