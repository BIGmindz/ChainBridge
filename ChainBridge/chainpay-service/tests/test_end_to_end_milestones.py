...
class TestEndToEndFreightLifecycle:
    ...
    def test_e2e_low_risk_freight_full_lifecycle(self, client: TestClient, db_session: Session):
        ...
        pickup_event = ShipmentEventWebhookRequest(
            shipment_id=freight_token_id, event_type="PICKUP_CONFIRMED", occurred_at=datetime.utcnow(), event_id=1001
        )
        ...
        pod_event = ShipmentEventWebhookRequest(
            shipment_id=freight_token_id, event_type="POD_CONFIRMED", occurred_at=datetime.utcnow(), event_id=1002
        )
        ...
        claim_event = ShipmentEventWebhookRequest(
            shipment_id=freight_token_id, event_type="CLAIM_WINDOW_CLOSED", occurred_at=datetime.utcnow(), event_id=1003
        )
    ...
    def test_e2e_medium_risk_freight_lifecycle(self, client: TestClient, db_session: Session):
        ...
        for event_type, expected_amount in events:
            event = ShipmentEventWebhookRequest(
                shipment_id=freight_token_id, event_type=event_type, occurred_at=datetime.utcnow(), event_id=None
            )
    ...
    def test_e2e_high_risk_freight_lifecycle(self, client: TestClient, db_session: Session):
        ...
        pickup_event = ShipmentEventWebhookRequest(
            shipment_id=freight_token_id, event_type="PICKUP_CONFIRMED", occurred_at=datetime.utcnow(), event_id=None
        )
        ...
        pod_event = ShipmentEventWebhookRequest(
            shipment_id=freight_token_id, event_type="POD_CONFIRMED", occurred_at=datetime.utcnow(), event_id=None
        )
        ...
        claim_event = ShipmentEventWebhookRequest(
            shipment_id=freight_token_id, event_type="CLAIM_WINDOW_CLOSED", occurred_at=datetime.utcnow(), event_id=None
        )
    ...
    def test_e2e_idempotency_duplicate_events_ignored(self, client: TestClient, db_session: Session):
        ...
        event1 = ShipmentEventWebhookRequest(
            shipment_id=freight_token_id, event_type="PICKUP_CONFIRMED", occurred_at=datetime.utcnow(), event_id=1
        )
        ...
        assert response2.status_code in [200, 409], "Duplicate event should be idempotent"

        # Verify only 1 milestone created despite 2 requests
        db_session.refresh(payment_intent)
        milestones = (
            db_session.query(MilestoneSettlement)
            .filter(MilestoneSettlement.payment_intent_id == payment_id, MilestoneSettlement.event_type == "PICKUP_CONFIRMED")
            .all()
        )
    ...
    def test_e2e_audit_endpoints_verify_balances(self, client: TestClient, db_session: Session):
        ...
        for i, event_type in enumerate(["PICKUP_CONFIRMED", "POD_CONFIRMED", "CLAIM_WINDOW_CLOSED"]):
            event = ShipmentEventWebhookRequest(
                shipment_id=freight_token_id, event_type=event_type, occurred_at=datetime.utcnow(), event_id=i + 1
            )
    ...
    def test_e2e_sequential_multiple_intents(self, client: TestClient, db_session: Session):
        ...
        for i, (freight_id, payment_id, amount) in enumerate(intents):
            event = ShipmentEventWebhookRequest(
                shipment_id=freight_id, event_type="POD_CONFIRMED", occurred_at=datetime.utcnow(), event_id=i + 1
            )
            response = client.post("/webhooks/shipment-events", json=event.model_dump(mode="json"))
            assert response.status_code == 200
