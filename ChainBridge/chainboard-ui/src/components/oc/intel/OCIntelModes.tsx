...
export default function OCIntelModes({ modes }: OCIntelModesProps) {
  const totalShipments = modes.reduce(
    (sum, m) => sum + (m.shipment_count ?? m.activeShipments ?? 0),
    0
  );
  ...
  return (
    <div className="rounded-lg border border-slate-700 bg-slate-800/50">
      ...
      <div className="divide-y divide-slate-700/50">
        {modes.map((mode) => {
          const shipmentCount = mode.shipment_count ?? mode.activeShipments ?? 0;
          const riskDistribution =
            mode.risk_distribution ?? { low: 0, medium: 0, high: 0, critical: 0 };
          const volumePct =
            totalShipments > 0 ? ((shipmentCount / totalShipments) * 100).toFixed(1) : '0.0';
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {shipmentCount} shipments · {volumePct}% of total
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono font-semibold text-slate-200">
                    {formatCurrency(mode.valueUsd ?? 0)}
                  </span>
                </div>
                ...
                <div className="mt-2">
                  ...
                  <div className="flex h-2 overflow-hidden rounded-full bg-slate-700">
                    <div
                      ...
                      style={{
                        width: `${((riskDistribution.low / Math.max(shipmentCount, 1)) * 100).toFixed(0)}%`,
                      }}
                    />
                    <div
                      className="bg-amber-400"
                      style={{
                        width: `${((riskDistribution.medium / Math.max(shipmentCount, 1)) * 100).toFixed(0)}%`,
                      }}
                    />
                    <div
                      className="bg-orange-500"
                      style={{
                        width: `${((riskDistribution.high / Math.max(shipmentCount, 1)) * 100).toFixed(0)}%`,
                      }}
                    />
                    <div
                      className="bg-red-500"
                      style={{
                        width: `${((riskDistribution.critical / Math.max(shipmentCount, 1)) * 100).toFixed(0)}%`,
                      }}
                    />
                  </div>
