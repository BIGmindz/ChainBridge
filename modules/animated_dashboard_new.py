...
class DashboardRenderer:
    ...
    def render_sidebar(self):
        ...
        st.sidebar.metric(state_cfg['primary_metric_label'], f"${latest_row[state_cfg['primary_metric_col']]:,.2f}", f"{performance_metric:,.2f} ({perf_pct:.2f}%)")
        
        state_items = {col.replace(state_cfg['state_item_prefix'], ''): latest_row[col] 
                       for col in self.data.columns if col.startswith(state_cfg['state_item_prefix'])}
    ...
    def render_entity_activity_chart(self, entity):
        ...
        df_resampled = df_entity.set_index(cfg['data_source']['timestamp_col'])[cfg['data_source']['value_col']].resample('1T').ohlc().dropna()
        ...
        if cfg['visualization']['chart_type'] == 'candlestick':
            fig.add_trace(go.Candlestick(x=df_resampled.index, open=df_resampled['open'], high=df_resampled['high'], low=df_resampled['low'], close=df_resampled['close'], name='Value', increasing_line_color='#2EBE7B', decreasing_line_color='#D14E55'), row=1, col=1)
        else: # Default to line chart
            fig.add_trace(go.Scatter(x=df_resampled.index, y=df_resampled['close'], name='Value', line=dict(color='#00A8CC')), row=1, col=1)

        fig.add_trace(go.Scatter(x=df_resampled.index, y=df_resampled['ma_fast'], name=f"MA({cfg['visualization']['fast_ma_period']})", line=dict(color='#00A8CC', width=1)), row=1, col=1)
        fig.add_trace(go.Scatter(x=df_resampled.index, y=df_resampled['ma_slow'], name=f"MA({cfg['visualization']['slow_ma_period']})", line=dict(color='#FFC107', width=1)), row=1, col=1)
        fig.add_trace(go.Scatter(x=positive_events[cfg['data_source']['timestamp_col']], y=positive_events[cfg['data_source']['value_col']], name='Positive Events', mode='markers', marker=dict(color='#2EBE7B', size=10, symbol='triangle-up', line=dict(width=1, color='White'))), row=1, col=1)
        fig.add_trace(go.Scatter(x=negative_events[cfg['data_source']['timestamp_col']], y=negative_events[cfg['data_source']['value_col']], name='Negative Events', mode='markers', marker=dict(color='#D14E55', size=10, symbol='triangle-down', line=dict(width=1, color='White'))), row=1, col=1)
        ...
        fig.update_layout(title=f"{entity} Activity Analysis", template="plotly_dark", paper_bgcolor='#0E1117', plot_bgcolor='#1A1C22', xaxis_rangeslider_visible=False, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    ...
    def render_kpis_and_events(self):
        ...
        with col2:
            ...
            wins = len(self.data[self.config['state_tracking']['performance_metric_col']] > self.data[self.config['state_tracking']['performance_metric_col']].shift(1).fillna(0))
