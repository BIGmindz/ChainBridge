    ...
    def _load_or_generate_data(self) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            if os.path.exists(f"{self.data_path}/port_congestion.csv"):
                data["port_congestion"] = pd.read_csv(f"{self.data_path}/port_congestion.csv", 
                                                    parse_dates=["date"])
            
            if os.path.exists(f"{self.data_path}/diesel_prices.csv"):
                data["diesel_prices"] = pd.read_csv(f"{self.data_path}/diesel_prices.csv", 
                                                   parse_dates=["date"])
                
            if os.path.exists(f"{self.data_path}/container_rates.csv"):
                data["container_rates"] = pd.read_csv(f"{self.data_path}/container_rates.csv", 
                                                     parse_dates=["date"])
...
if __name__ == "__main__":
    ...
    print(f"Traditional signals correlation: {correlation_map['traditional_signals']['correlation']:.2f} ({correlation_map['traditional_signals']['edge']})")
    print(f"Logistics signals correlation: {correlation_map['logistics_signals']['correlation']:.2f} ({correlation_map['logistics_signals']['edge']})")
