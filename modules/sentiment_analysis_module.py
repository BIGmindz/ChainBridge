...
class SentimentAnalysisModule(Module):
    ...
    def calculate_composite_sentiment(self, alternative_data: Dict[str, float], 
    ...
    def generate_signal(self, composite_score: float, individual_scores: Dict[str, float],
                       current_price: float = None, price_trend: str = None) -> Tuple[str, float, str]:
        """
        ...
        consensus_factors = len([score for score in individual_scores.values() 
                               if (score > 0.2 and composite_score > 0) or 
                                  (score < -0.2 and composite_score < 0)])
