...
class RegimeBacktester:
    ...
    def _detect_regimes(self) -> None:
        ...
        if not self.regime_labels:
            self.regime_labels = [
                "Ranging", "Bullish", "Bearish", 
                "Choppy", "Bullish Volatile", "Bearish Volatile"
            ]
    ...
    def run_backtest(
        ...
        overall_returns = self._calculate_returns(self.price_data, 
                                                strategy_fn(self.price_data, params))
    ...
    def plot_results(self, figsize: Tuple[int, int] = (12, 8)) -> None:
        ...
        if self.regime_data is not None and len(self.regime_labels) > 0:
            ...
            labels = [self.regime_labels[int(r)] if int(r) < len(self.regime_labels) else f"Regime {r}" 
