            ...
            if self.regime_aware:
                ...
                if len(self.regime_history) > 1:
                    ...
                    if prev_regime != self.current_regime:
                        print(
                            f"📊 Market regime change detected: {prev_regime.value} → {self.current_regime.value} "
    ...
    def update_ml_weights(self, trade_result: Dict):
        ...
        if pnl > 0:
            ...
            consecutive_losses = sum(
                1 for t in self.trade_history[-3:] if t.get("pnl", 0) < 0
            )
