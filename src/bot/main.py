...
# Enterprise imports
from src.bot.core.secrets_manager import get_secrets_manager, SecretsManager
from src.bot.core.observability import get_observability, init_observability, ObservabilityManager
...
""")
    ...
    def validate_environment(self) -> bool:
        ...
        with self.observability.measure_operation("environment_validation"):
            ...
            try:
                credentials = self.secrets.get_exchange_credentials('kraken')
                print(f"✅ Exchange credentials: Available")
        ...
        for secret_key in secrets_to_check:
            if self.secrets.check_rotation_needed(secret_key, max_age_days=30):
                self.observability.logger.warning(
                    f"Secret rotation needed: {secret_key}",
                    secret=secret_key
                )
