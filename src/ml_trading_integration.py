...
class MLTradingIntegration:
    ...
    def __init__(self, paper_bot: KrakenPaperLiveBot, module_manager: ModuleManager, config: Dict[str, Any]):
        """
    ...
    def get_signal_performance_report(self) -> Dict[str, Any]:
        ...
        if success_rates:
            ...
            if avg_module_performance:
                best_module = max(avg_module_performance.keys(), key=lambda x: avg_module_performance[x])
                worst_module = min(avg_module_performance.keys(), key=lambda x: avg_module_performance[x])
...
async def create_ml_trading_system(config_path: str = None) -> Tuple[KrakenPaperLiveBot, MLTradingIntegration]:
    """
...
async def run_integrated_trading_loop(paper_bot: KrakenPaperLiveBot, 
