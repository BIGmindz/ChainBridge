...
KRAKEN_SYMBOL_MAP = {
    'XXBT': 'BTC', 'XBT': 'BTC', 'XETH': 'ETH', 'XDG': 'DOGE', 'XXRP': 'XRP', 'XLTC': 'LTC',
    'XTRX': 'TRX', 'XADA': 'ADA', 'XXLM': 'XLM', 'XETC': 'ETC'
}
...
def main():
    ...
    args = ap.parse_args(); target = args.amount
    if target <= 0:
        print('Amount must be positive'); return 1
    ex = load_exchange()
    print(f"🌐 Exchange time: {ex.iso8601(ex.milliseconds())}")
    assets = fetch_assets(ex)
    if not assets:
        print('No sellable assets (only stable or zero balances).'); return 0
