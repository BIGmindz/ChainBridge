...
class EnhancedMultiSignalBot:
    ...
    def __init__(self, config_path: str = "config.yaml", enhanced_config_path: str = "config/kraken_paper_trading.yaml"):
        """
    ...
    async def initialize_exchange(self):
        ...
        class MockExchange:
            def fetch_ticker(self, symbol):
                prices = {
                    'BTC/USD': {'last': 45000, 'bid': 44950, 'ask': 45050, 'quoteVolume': 1500000, 'timestamp': datetime.now().timestamp() * 1000},
                    'ETH/USD': {'last': 3200, 'bid': 3195, 'ask': 3205, 'quoteVolume': 800000, 'timestamp': datetime.now().timestamp() * 1000}
                }
                return prices.get(symbol, {'last': 100, 'bid': 99.5, 'ask': 100.5, 'quoteVolume': 100000, 'timestamp': datetime.now().timestamp() * 1000})
            
            def fetch_ohlcv(self, symbol, timeframe, limit=100):
                # Return mock OHLCV data
                return [[datetime.now().timestamp() * 1000, 45000, 45100, 44900, 45050, 1000] for _ in range(limit)]
